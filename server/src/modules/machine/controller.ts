...
export const getMachineConfiguration = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
) => {
  ...
  try {
    ...
    if(!Machine){
      ...
      return res.status(200).json({ success: true, message: "Machine status updated successfully", Machine: Machine });
    }
  } catch (err) {
    next(err);
  }
...
export const changeMachineStatus = async (
  ...
  try {
    ...
    const updateMachine = await prisma?.hostMachine.update({
      where: {
        id : machineId as string
      },
      data: {
    ...
    if(!updateMachine){
      return res.status(500).json({ success: false, message: "Failed to update machine status" });
    }else{
      return res.status(200).json({ success: true, message: "Machine status updated successfully", Machine: updateMachine });
    }
  } catch (err) {
    next(err);
  }
